...
def stage_data(entry,dataS,rawData,StageData,StageColors,medians,lengths):
    #stages
    ...
    P = Figure(figsize=(5,4), dpi=100)
    
    length = lengths*.5
    if(len(StageOne) != 0):
        idsone = [x for x in range(len(StageOne))]
        P.add_subplot(idsone,sorted(StageOne),color=stage1color,label="Stage One")
        if(medians == 1):
            midlinex = [0,length]
            med1 = [median(StageOne),median(StageOne)];
            P.add_subplot(midlinex,med1,color=stage1color,linestyle="--")
