...
BioGap, via data from The Cancer Genomic Atlas rapidly creates standard statistical models to evaluate possible on oncogenic drivers.

Through the use of expression data combined with choices of seperation by: stage data, sex type, age range, and races.
The expression data gathered can be used to preform several different graph types: corelation, heatmap correlation, "Standard" kaplan meier curves, Optimized kaplan meier curves, stage-expresion graph, and just expression graphs.
The selected data will always calculate into: average, standard deviation, range, minimum, maximum, first quartile, second quartile, thired quartile, median,range, and interquartile range

for more help go to file at the top left corner and select help or use the Help.pdf file in the main folder.

Necessary Files to have in repository:

    bestkaplansalltwoline.py
    bestkaplanstwoline.py
    BESTKAPS.py
    CORRELATEALL.py
    correlatetcga.py
    heatmap_cellline_expression.py
    heatmapforexpression.py
    heatmapforexpressionncbi.py
    kaplan_change.py
    Kaplan_Meier_Graphing.py
    Kaplan_Meier_Graphing_1_10_lines.py
    Kaplan_Meier_Graphing_expected.py
    ncbi_cellline_search.py
    ncbi_correlate.py
    ncbi_data_pull.py
    prc.py
    stage.py
...
Necessary Oxilary Files:
    Help.pdf

"""
__author__      = "Aaron Schwan"
__author__      = "AaronSchwan2018"
...
class Window(Frame):
    ...
    def init_window(self):
        ...
        def client_exit():
            exit()
        def client_help():
            #subprocess.Popen(["/home/aaron/Desktop/BioGAPALPHA 2.45/Help.pdf"],shell=True)
            #os.filestart("/home/aaron/Desktop/BioGAPALPHA 2.45/Help.pdf")
            os.system("/usr/bin/gnome-open /home/aaron/Desktop/BioGAPALPHA 2.45/Help.pdf");
        ...
        load = Image.open("/home/aaron/Desktop/BioGAPALPHA 2.45/Program Setup/Pictures/Title.png")
        ...
        def ncbiT_search():
            ...
            if(len(urls) > 0 and prob != 'Search Probe Number'):      
        ...
        def gene_search_tcga():
            ...
            StageData,rawData,RaceData,SexData,Survival,Censored,AliveDead,AgeData = tcga_genesearch.geneSearch(entry,outliers,dataS,STAGES,AGES,SEXES,RACES)
            print(StageData,rawData,RaceData,SexData,Survival,Censored,AliveDead,AgeData)
...
##            
        ...
        def NCBITUMOR():
            ...
            global geneSearch
            geneSearch = Entry()
            geneSearch.insert(0,"Search Probe Number")
            geneSearch .configure(width = 25, relief = FLAT)
            geneSearch_window = canvas.create_window(525, 15, anchor=NW,tag = 'all',  window=geneSearch)
            ...
            global geneCSearch
            geneCSearch = Entry()
            geneCSearch.insert(0,"Correlate to Probe")
            geneCSearch .configure(width = 24)
            geneCSearch_window = canvas.create_window(100, 295, anchor=NW, window=geneCSearch)
        ...
        def NCBICELL():
            ...
            global SearchProbE
            SearchProbE = Entry()
            SearchProbE.insert(0,"Probe Number")
            ...
            Button2 = Button(text = "Search Probe", command = search_prob, anchor = W)
            ...
            global heatmapSearch
            heatmapSearch = Entry()
            heatmapSearch.insert(0,'Heat Map Probe Number')
